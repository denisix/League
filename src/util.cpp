...
int nAnonymizeLeagueAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\League
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\League
    // Mac: ~/Library/Application Support/League
    // Unix: ~/.League
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "League";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "League.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Leagued.pid"));
