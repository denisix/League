...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: League.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Leagued.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeLeagueamount=<n> " + _("Keep N League anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. League is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. League is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("League version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "League server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of League") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart League to complete") << "\n";
    ...
    nAnonymizeLeagueAmount = GetArg("-anonymizeLeagueamount", 0);
    if(nAnonymizeLeagueAmount > 999999) nAnonymizeLeagueAmount = 999999;
    if(nAnonymizeLeagueAmount < 2) nAnonymizeLeagueAmount = 2;
    ...
    LogPrintf("Anonymize League Amount %d\n", nAnonymizeLeagueAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1LGA+1000 == (.1LGA+100)*10
       10LGA+10000 == (1LGA+1000)*10
    */
