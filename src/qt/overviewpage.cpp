...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeLeagueAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLeagueAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeLeagueAmount = strAnonymizeLeagueAmount.remove(strAnonymizeLeagueAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLeagueAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeLeagueAmount*COIN) nMaxToAnonymize = nAnonymizeLeagueAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeLeagueAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeLeagueAmount));
        strAnonymizeLeagueAmount = strAnonymizeLeagueAmount.remove(strAnonymizeLeagueAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLeagueAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
