...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLeagueAmount"))
        settings.setValue("nAnonymizeLeagueAmount", 1000);
    nAnonymizeLeagueAmount = settings.value("nAnonymizeLeagueAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeLeagueAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeLeagueAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeLeagueAmount:
            return QVariant(nAnonymizeLeagueAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeLeagueAmount:
            nAnonymizeLeagueAmount = value.toInt();
            settings.setValue("nAnonymizeLeagueAmount", nAnonymizeLeagueAmount);
            emit AnonymizeLeagueAmountChanged(nAnonymizeLeagueAmount);
            break;
        default:
            break;
        }
